...
At Interlock, we keep everything in one repo. The only exception is when we're working on something confidential since GitHub does not allow confidential issues inside public repos. Here's why:
...
- One repo pools GitHub stars and more accurately reflects Interlock’s presence.
...
Every group at Interlock maintains specific Discord channels, which all group members join and keep unmuted.  Everyone else at Interlock is encouraged to mute these channels, using them only as needed.  Each channel has a directly responsible individual responsible for keeping up with all new messages, even if they aren't explicitly mentioned (`@`).


## Development groups

Interlock organizes development groups by their goals. These include members from Design, Engineering, and Product.
...
- **Agent**: grow # open source, osquery-based agents by making Interlock’s agents better, faster, and broader in capabilities
  - (+) every table works intuitively with user-friendly docs and empathetic caveats, warnings, and error messages

At Interlock, groups define the relevant sections of the engineering org chart.  A product manager (PM) represents each group and reports to the Product department (or a founder serving as an
...
### Interface group
#### Responsibilities
- Everything related to Interlock's graphical user interface (other than for the desktop application portion of Interlock Desktop)
- `fleetctl` (the Interlock command-line interface) and the associated YAML documents (almost everything in fleetctl besides the `fleetctl debug` subcommands)
...
- Interlock's configuration surface, including
  - The config settings that exist for Interlock deployments and how they're configured
  ...
  - The UX of upgrading and downgrading and sidegrading Interlock tiers, and managing license keys

#### Consumers
- A human using Interlock's graphical user interface
- A human who is writing code that integrates Interlock's REST API 
- A human reading Interlock's REST API docs
- A human using fleetctl, Interlock's Postman collection or Interlock's other future SDK wrappers

These humans might be working within the "Interface" group itself insofar as they consume the Interlock REST API.  Or they might be a contributor to the Interlock community.  Or one of Interlock's core users or customers, usually in an SRE, IT, or security role in an organization.

#### Goals
- Bring value to Interlock users by delivering new features and iterations of existing features.
...
- Ensure observance of semantic versioning for the Interlock API and config between releases so that only major versions include breaking changes.
- Delight users of Interlock's API, UI, SDKs, and documentation with a simple, secure, widely-adopted user and developer experience.
- **Improve Interlock’s feature value and ease of use.**

### Platform group
#### Responsibilities
- The implementation of Interlock Agent API: i.e.,
  - The API used by agents on enrolled hosts to communicate with Interlock
  - The API used by agents for doing auto-updates and future installation/upgrade of custom extensions (e.g., TUF)
- Everything related to providing a stable, simple-to-build-on platform for Interlock contributors to use when implementing changes to the REST API
...
- Production infrastructure, including
  - Interlock Cloud demo
  - Interlock Cloud prod
  - The registry (TUF) used for auto-updates and (in the future) extensions
  - whatever backend is needed to generate installers in self-managed and hosted Interlock deployments
  - The future monitoring and 24/7/365 enhancement to the on-call rotation necessary for Interlock Cloud
- Behind-the-scenes integrations
  - i.e., integrations that make Interlock "just work" and don't involve configuration from users
  - Example: the code that fetches and manages CVE data from NVD and other behind-the-scenes infrastructure that enables vulnerability management to exist in Interlock without requiring any interactions or configuration from users
- The CI/CD pipeline
...
- A host enrolled in Interlock running an osquery-based agent.
- The person who deploys, upgrades, and operates Interlock.
- A person who uses Interlock and expects it to be fast, reliable, and joyful.

#### Goals
- Reduce REST API latency
- Increase max load test size
- Reduce infrastructure costs for Interlock deployments.
- Make upgrading Interlock versions seamless for users.
...
- **Make Interlock as easy as possible to operate and contribute to**
...
- Orbit
- Interlock Desktop
- Any future agent-based software built by Interlock.
- Interlock Agent API: the API interface and contributor docs used by osquery-enrolled agents for communicating with Interlock  (how to implement the internals is up to the "Platform" group).
- The extensions/tables are bundled in Orbit/Interlock Desktop, such as mac admins.
...
- An end-user running an osquery-based agent (Interlock Desktop, orbit, or osquery) on their work laptop, who wants their laptop to be stable, performant, and as private as possible
...
- A contributor working on Interlock Desktop or orbit in fleetdm/fleet
- Interlock itself, consuming the data generated by osquery and any other agent software

#### Goals
- Grow mind/market share of open source, osquery-based agents by making Interlock’s agents better, faster, and broader in capabilities
- Every table works intuitively with user-friendly docs and empathetic caveats, warnings, and error messages
  - Every table is documented within the Interlock UI and in fleetdm.com/docs, with GOTCHAS, deprecation notices, and the version when the table was added
- **Make Interlock’s agents easy to operate and contribute to**
-->
...
There are several special email addresses that automatically relay messages to the appropriate people at Interlock. Each email address meets a minimum response time ("Min RT"), expressed in business hours/days, and has a dedicated, directly responsible individual (DRI) who is responsible for reading and replying to emails sent to that address.  You can see a list of those email addresses in ["Contacting Interlock" (private Google doc)](https://docs.google.com/document/d/1tE-NpNfw1icmU2MjYuBRib0VWBPVAdmq4NiCrpuI0F0/edit#).


## Tools we use

There are a number of tools that are used throughout Interlock. Some of these tools are used company-wide, while others are department-specific. You can see a list of those tools in ["Tools we use" (private Google doc)](https://docs.google.com/spreadsheets/d/170qjzvyGjmbFhwS4Mucotxnw_JvyAjYv4qpwBrS6Gl8/edit?usp=sharing).
...
The following Discord channels are maintained by Interlock's founders and executive collaborators:

| Discord channel               | [DRI](https://fleetdm.com/handbook/company#why-group-slack-channels)|
|:----------------------------|:--------------------------------------------------------------------|
